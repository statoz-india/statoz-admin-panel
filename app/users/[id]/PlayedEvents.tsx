...
type PlayedEventsProps = {
  userId: string;
};

/** Human label for the option the user bet on. */
function optionLabel(row: PlayedEvent): string {
  const e = row.event;
  if (row.chosenOption === "Y") return e?.yesPlaceholder ?? "Yes";
  if (row.chosenOption === "N") return e?.noPlaceholder ?? "No";
  return e?.maybePlaceholder ?? "Maybe";
}

export default function PlayedEvents({ userId }: PlayedEventsProps) {
  const [items, setItems] = useState<PlayedEvent[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError("");
      const res = await fetch(`/api/users/${userId}/playedEventsForAdmin`, {
        method: "GET",
      ...
      setError(e instanceof Error ? e.message : "Failed to load played events");
    } finally {
      setLoading(false);
    }
  }, [userId]);
  ...
  return (
    <div className="bg-zinc-900 rounded-lg border border-zinc-700 p-6">
      <div className="flex items-center justify-between gap-4 mb-4 flex-wrap">
        <h2 className="text-xl font-semibold text-white">Played events</h2>
        <button
          type="button"
          onClick={() => void load()}
          disabled={loading}
      ...
      {!loading && !error && items.length === 0 && (
        <p className="text-gray-400 text-sm">No event bets yet.</p>
      )}
            ...
            <tbody>
              {items.map((row) => (
                ...
                >
                  ...
                  <td className="py-3 pr-4">{optionLabel(row)}</td>
