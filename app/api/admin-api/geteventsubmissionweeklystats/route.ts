// curl -X GET "http://localhost:8000/api/v1/admin-data/getEventSubmissionWeeklyStats" \
//   -H "Content-Type: application/json" \
//   --cookie "accessToken=<superadmin_access_token>"
//
// Total event submissions, submissions this week, and weekly counts (IST).
...
export async function GET() {
  try {
    const response = await authenticatedFetch(
      "/admin-data/getEventSubmissionWeeklyStats",
    );
    ...
    console.error("Error fetching event submission weekly stats:", error);
    return NextResponse.json(
      {
        success: false,
        message: "Failed to fetch event submission weekly stats",
      },
      { status: 500 },
    );
  }
