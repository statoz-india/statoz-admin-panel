// curl -X GET "http://localhost:8000/api/v1/admin-data/getPredictionSubmissionStats" \
//   -H "Content-Type: application/json" \
//   --cookie "accessToken=<superadmin_access_token>"
//
// Total prediction submissions today + daily counts for the last 7 IST days.
...
export async function GET() {
  try {
    const response = await authenticatedFetch(
      "/admin-data/getPredictionSubmissionStats",
    );
    ...
    console.error("Error fetching prediction submission stats:", error);
    return NextResponse.json(
      {
        success: false,
        message: "Failed to fetch prediction submission stats",
      },
      { status: 500 },
    );
  }
