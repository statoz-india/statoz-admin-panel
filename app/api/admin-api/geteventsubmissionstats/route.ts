// curl -X GET "http://localhost:8000/api/v1/admin-data/getEventSubmissionStats" \
//   -H "Content-Type: application/json" \
//   --cookie "accessToken=<superadmin_access_token>"
//
// Total event submissions today + daily counts for the last 7 IST days.
...
export async function GET() {
  try {
    const response = await authenticatedFetch(
      "/admin-data/getEventSubmissionStats",
    );
    ...
    return NextResponse.json(
      {
        success: false,
        message: "Failed to fetch event submission stats",
      },
      { status: 500 },
    );
  }
